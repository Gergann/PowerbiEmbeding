...
//Retrieves all pages included in the report 
//returns: List of dictionaries {displayName, name, order}
async function getReportPages(workspaceId,reportId) {

    const ReportPagesApi = `https://api.powerbi.com/v1.0/myorg/groups/${workspaceId}/reports/${reportId}/pages`;
    const headers = await getRequestHeader();
    ...
    pages = [];
    for (page of resultJson.value) {
        //exclude Custom Tooltips
        if (!page.displayName.includes('Tooltip')){
        pages.push({ 'displayName': page.displayName, 'name': page.Name, 'order': page.order })
        }
    }
    return pages;
}

async function getEmbedTokenForAllDashboards(dashboardIds, datasetIds, targetWorkspaceId) {

    // Add report id in the request
    let formData = { 'dashboards': [] };
...
async function getEmbedParamsForAllDashboards(targetWorkspaceId, dataset_id, dashboard_ids) {

    const dashboardEmbedConfig = new EmbedConfig();

    dashboardEmbedConfig.reportsDetail = [];

    for (const dashboard of dashboard_ids) {
        const DashboardsInGroup = `https://api.powerbi.com/v1.0/myorg/groups/${targetWorkspaceId}/dashboards/${dashboard}`;
        const headers = await getRequestHeader();



        const result = await fetch(DashboardsInGroup, {
            method: 'GET',
            headers: headers,
        });
        ...
        // Add report data for embedding
        const dashboardDetails = new PowerBiReportDetails(resultJson.id, resultJson.displayName, resultJson.embedUrl);

        dashboardEmbedConfig.reportsDetail.push(dashboardDetails);
    }
    //dataset_id = [getDatasetIdFromReportId(report_id,targetWorkspaceId)]

    dashboardEmbedConfig.embedToken = await getEmbedTokenForAllDashboards(dashboard_ids, dataset_id, targetWorkspaceId);
    return dashboardEmbedConfig
}


async function getDashboards(targetWorkspaceId) {

    const DashboardsInGroup = `https://api.powerbi.com/v1.0/myorg/groups/${targetWorkspaceId}/dashboards`;
    const headers = await getRequestHeader();
    ...
    dashboards = [];
    for (dashboard of resultJson.value) {
        dashboards.push(dashboard.displayName)
    }
